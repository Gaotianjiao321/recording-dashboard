...
export async function chunkAudio(filePath, options = {}) {
  const chunkSeconds = options.chunkSeconds ?? Number(process.env.AUDIO_CHUNK_SECONDS ?? 240);
  const overlapSeconds = options.overlapSeconds ?? 5;
  const outputDir = options.outputDir ?? process.env.AUDIO_CHUNKS_DIR ?? "recordings/chunks";
